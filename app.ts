...
const gameCreation = (user: User, msg: Message) => {
    if (user.previousStage) user.data.game[user.previousStage.stage] = msg.text
    if (user.currentStage.stage != 'end') bot.sendMessage(user.id, user.currentStage.message)
        .then(() => user.stateUp())
}
